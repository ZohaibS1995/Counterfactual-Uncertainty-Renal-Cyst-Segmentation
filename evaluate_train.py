...
if __name__ == "__main__":
    ...
    checkpoint = args.checkpoint
    net = get_network(properties, task_id, r"/data/kidnAI/DynUnet/data/model_uncertain_fold0", 'fold0_uncertain_2_399.pth')    # fold2, 1_392  #alternate fold4 2_390  fold1 0_385
    net = net.to(device)
    net.eval()
...
#
    ...
    args.label_save_dir = r"/data/kidnAI/DynUnet/data/results/pred_fold0"
    datalist_name = "dataset_task{}.json".format(args.task_id)
    dataset_path = os.path.join(args.root_dir, task_name[task_id])
    datalist = load_decathlon_datalist(os.path.join(args.datalist_path, datalist_name), True, list_key, dataset_path)
    for item in datalist:
