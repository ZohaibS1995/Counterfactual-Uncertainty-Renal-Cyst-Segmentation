...
if __name__ == "__main__":
    ...
    checkpoint = args.checkpoint
    fold = 4
    net = get_network(properties, task_id, f"/data/kidnAI/DynUnet/data/model_uncertain_fold{fold}", f'fold0_uncertain_2_399.pth')
    list_key = "{}_fold{}".format("validation", fold)  # "test" #
    args.label_save_dir = f"/data/kidnAI/DynUnet/data/results/pred_fold{fold}"
    net = net.to(device)
    net.eval()
    ...
    #args.label_save_dir = r"/data/kidnAI/DynUnet/data/results/pred_labelsTs"
    ...
    path_malignant_imagesTr =r"/data/kidnAI/DynUnet/data/Malignant_cases/imagesTr_final"
    path_malignant_predTr = r"/data/kidnAI/DynUnet/data/Malignant_cases/predTr"

    img_names = glob.glob(os.path.join(path_malignant_imagesTr, '*.nii.gz'))
    for t_img in tqdm(img_names):
        test_img = t_img
        ...
        sitk.WriteImage(test_pred_lab_sitk, os.path.join(path_malignant_predTr, test_img.split("/")[-1].replace("_0000.nii.gz", ".nii.gz")), useCompression=True)
